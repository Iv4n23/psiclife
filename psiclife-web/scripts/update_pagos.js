...
const startPattern = /    const pagado    = detalle.pagos\?\.reduce\(\(acc, p\) => acc \+ Number\(p.monto\), 0\) \?\? 0/;
...
const endPattern = /    \)/;
// search from start index to end of block
...
const endBlock = remaining.indexOf('    )');

const before = content.slice(0, startIndex);
const after = content.slice(startIndex + endBlock + 5);
